...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority, ContentProviderClient provider, SyncResult syncResult) {
        // actual syncing will begin once the client is connected
        // see onConnected
        mApiClient.connect();
    }
    ...
    private void updateWear() {
        Log.d(TAG, "updateWear() called with: " + "");
        new Thread(new Runnable() {
            @Override
            public void run() {
                if (!mApiClient.isConnected() && !mApiClient.isConnecting()) {
                    Log.i(TAG, "run: not connected and not connecting");
                    // if the client is not connected and the client is not connecting, something is wrong
                    // do nothing
                    return;
                }

                if (mApiClient.isConnected()) {
                    // if the client is connected, send the data
                    Log.i(TAG, "run: is connected");
                    updateWearData();
                } else {
                    while (!mApiClient.isConnected() && mApiClient.isConnecting()) {
                        try {
                            // the client is still connecting, give it some time to connect
                            wait(200);
        ...
        Wearable.NodeApi.getConnectedNodes(mApiClient).setResultCallback(new ResultCallback<NodeApi.GetConnectedNodesResult>() {
            ...
            public void onResult(NodeApi.GetConnectedNodesResult getConnectedNodesResult) {
                for (Node node : getConnectedNodesResult.getNodes()) {
                    PendingResult<MessageApi.SendMessageResult> sendMessageResultPendingResult = Wearable.MessageApi.sendMessage(
                            mApiClient, node.getId(), "/dataUpdated", (mLastWearData.toByteArray()));
                    sendMessageResultPendingResult.setResultCallback(new ResultCallback<MessageApi.SendMessageResult>() {
                        @Override
                        public void onResult(MessageApi.SendMessageResult sendMessageResult) {
                            Log.i(TAG, "onResult: " + sendMessageResult.getStatus());
                        }
                    });
    ...
    private class DoWorkAsyncTask extends AsyncTask<Void, Void, Void> {
